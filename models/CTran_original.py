...
class CTranModel(nn.Module):
    def __init__(self, num_labels, use_lmt, device, backbone_model, pos_emb=False, layers=3, heads=4, dropout=0.1,
                 int_loss=0, no_x_features=True, grad_cam=False):
    ...
    def forward(self, images, mask=None):
        ...
        if self.use_pos_enc:
            pos_encoding = self.position_encoding(features, torch.zeros(features.size(0), 18, 18, dtype=torch.bool).to(
                self.device))
